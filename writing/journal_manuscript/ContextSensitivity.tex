...
This phenomenon is most immediately motivated by recent findings that more informative or diagnostic words in the local referential context are significantly more likely to become conventionalized: words from initial descriptions like ``skater,'' which are only used with a single referent, are more likely to persist in the resulting convention than a word like ``man'' which is used with multiple referents.
However, our broader theoretical aim is to suggest that \emph{diachronic} patterns toward efficiency and informativity in the long-term evolution of a community's lexical conventions, as highlighted by the Optimal Semantic Expressivity (OSE) hypothesis \cite{frankblogpost} and functionalist theories more broadly, may be explained as a result of the \emph{synchronic} processes at play when individual dyads coordinate on  \emph{ad hoc} meanings.
