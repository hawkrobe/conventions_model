...
We ran 50 distinct trajectories with parameter settings of $\alpha_L=8, \alpha_S=8$ and memory discounting parameter of $\beta = 0.6$.% (see Model comparison below for a more thorough evaluation of these parameters).
...
We recruited 278 participants from Amazon Mechanical Turk to play an interactive, multi-player game. Pairs were randomly assigned to one of three different conditions, yielding $n=36$ dyads in the \emph{coarse} condition, $n=38$ in the \emph{fine} condition, and $n=53$ in the \emph{mixed} condition after excluding participants who disconnected before completion\footnote{This experiment was pre-registered at \url{https://osf.io/2hkjc/} with a target sample size of roughly 40 games per condition. We had originally also planned to exclude participants who failed to achieve 75\% accuracy on the final block, to restrict analyses to ``converged'' lexica, but later decided that this would eliminate variation of interest across conditions, and instead included all pairs who completed the task. All statistical tests in mixed-effects models reported in this section use degrees of freedom based on the Satterthwaite approximation \cite{luke2017evaluating}.}.
...
But did these lexica emerge over the course of interaction? 
...
As in the post-test reports, we found an overall main effect of condition, with participants in the coarse condition using significantly fewer words throughout the experiment. 
