...
Our explanation recalls classic observations about \emph{hedges}, expressions like \emph{sort of} or \emph{like}, and morphemes like \emph{-ish}, that explicitly mark provisionality, such as \emph{a car, sort of silvery purple colored} \cite{Fraser10_Hedging,MedlockBriscoe07_HedgeClassification} \adele{pl add Lakoff, G. (1975). Hedges: A study in meaning criteria and the logic of fuzzy concepts. In Contemporary research in philosophical logic and linguistic semantics (pp. 221-271). Springer, Dordrecht (would be a bad oversight for me to not include w nearly 4K cites)}.
...
%This is not the case with a classical truth-functional lexical representation, but
as can be formalized naturally with soft semantics (see Appendix C).
...
\adele{do you mean, whole NPs are dropped rather than just the determiner? If so, could say, "entire modifying clauses are more likely to be dropped at once than would be expected by random corruption of noun phrase or prepositional phrase modifiers"...Grammatical elements do get reduced via repetition and can be eliminated;  iced tea > ice tea; skimmed milk > skim milk; or in fly-fishing". It's really that as you note, modification becomes unnecessary and a simple referring expression suffices}
...
%nice! Does the modeling with 2 word descriptions rather than say a construction with two parts: modifying and referring capture this? could be nice to do in future work: which parts of utterances are likely to get reduced or enhanced (in the case of subordinate level descriptions) and why?
