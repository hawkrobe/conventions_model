...
These reference games used a set of two objects $\{o_1, o_2\}$ and four utterances $\{u_1, \dots, u_4\}$ as in Simulation 1.2; agents were randomized to roles when assigned to a new partner and swap roles after each repetition block.
...
Following \citeA{KempPerforsTenenbaum07_HBM}, we extend the uniform categorical prior over objects to a hierarchical Dirichlet-Multinomial model \cite{gelman_bayesian_2014}, where the prior probability of partner-specific meaning $\phi_k(u) = o_i$ is not uniform, but parameterized by probabilities $\theta_k$ drawn from a population-level Dirichlet distribution:
\begin{equation*}
\begin{array}{rcl}
\phi_k(u)  &\sim&  \textrm{Categorical}(\theta_k) \\
\theta_k  & \sim & \textrm{Dirichlet}(\Theta)\\
\end{array}
\end{equation*}
...
Because agents have uncertainty about these population-level properties and are attempting to learn them from interactions with individual partners, we assume fixed hyper-prior $P(\Theta) = P(\boldsymbol{\alpha})P(\lambda)$ where the hyper-prior for $ \boldsymbol{\alpha} $ may be slightly biased from uniform for different utterances, as in Simulation 1.2 above:


\begin{equation*}
\begin{array}{rcl}
 \boldsymbol{\alpha} & \sim & \textrm{Dirichlet}(\boldsymbol{1}_{\pm\delta})\\
\lambda & \sim & \textrm{Exponential}(1).
\end{array}
\end{equation*}
...
For example, in the first block, agents 1 and 2 may coordinate on using $u_1$ to refer to $\orangecircle$ while agent 3 and 4 coordinate on using $u_2$. 
