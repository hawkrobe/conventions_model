...
\begin{figure*}
\includegraphics[scale=1.05]{./figures/sec3-model-results.pdf}
\vspace{1em}
...
\end{array}
\end{equation*}
where $\boldsymbol{\alpha}$ is the Dirichlet concentration parameter encoding the agent's beliefs, or ``over-hypotheses'' about both the central tendency and the variability of lexicons in the population. 
...
We assume the agent also has uncertainty about these population-level properties and place a hyper-prior on $\alpha$ \cite<see>{cowans2004information}:
