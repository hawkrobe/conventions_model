...
Experience with a single partner ought to be relatively uninformative about further partners, hence our partial-pooling account behaves much like a no-pooling model in predicting strong partner-specificity and discounting outliers \cite{dautriche2021}.
...
Following \citeA{KempPerforsTenenbaum07_HBM}, we extend the uniform categorical prior over objects to a hierarchical Dirichlet-Multinomial model \cite{gelman_bayesian_2014}, where the prior probability of partner-specific meaning of $u$, $\phi_k^u~=~o_i$, is not uniform, but parameterized by probabilities $\theta_k$ drawn from a population-level Dirichlet distribution:
\begin{equation*}
\begin{array}{rcl}
\phi_k^{u}  &\sim&  \textrm{Categorical}(\theta_k^{u}) \\
\theta_k^{u}  & \sim & \textrm{Dirichlet}(\Theta^{u})\\
\end{array}
\end{equation*}

The shared Dirichlet distribution is defined by a \emph{concentration} parameter $\Theta$ which is factored into the product of two terms, $\Theta = \lambda \cdot \boldsymbol{\alpha}$.
The vector $\boldsymbol{\alpha}$ represents the relative probabilities of meanings and the scalar $\lambda$ represents the concentration's magnitude: when $\lambda$ is small, the Dirichlet tends to sample more binary distributions (e.g. $p_i$ near 0 or 1) and when $\lambda$ is larger, the Dirichlet tends toward weaker, uniform distributions (e.g. $p_i$ near $1/|\mathcal{O}|)$.
With these components, $\Theta$ therefore encodes both the central tendency and the variability of the population. 
Because agents have uncertainty about these population-level properties and are attempting to learn them from interactions with individual partners, we assume fixed hyper-prior $P(\Theta^u) = P(\boldsymbol{\alpha}^u)P(\lambda)$ where the hyper-prior for $ \boldsymbol{\alpha}^u$ is again slightly biased from uniform for different utterances, as in Simulation 1.2 above:


\begin{equation*}
\begin{array}{rcl}
 \boldsymbol{\alpha}^u & \sim & \left\{
\begin{array}{rcl}
  \textrm{Dirichlet}(2,3)& \textrm{if} & u \in \{u_1, u_2\}\\
  \textrm{Dirichlet}(3,2) & \textrm{if} &u \in \{u_3, u_3\}\\
\end{array}\right. \\
\lambda & \sim & \textrm{Exponential}(.5).
\end{array}
\end{equation*}

It is then clear how no-pooling and complete-pooling models lesion this shared structure in different ways. 
The no-pooling model simply lesions the Dirichlet prior entirely and learns a separate categorical distribution $\theta_k$ for each partner.
The complete-pooling model, on the other hand, samples lexicons directly from a categorical parameterized by the shared $\Theta$ without allowing variability in $\theta_k$ for each partner.
We simulated 48 networks, setting $w_S = 11,~w_C = .2$ (see Fig.~\ref{fig:partnerspecificity_grid} in the Appendix for an exploration of other parameters).
...
We measured alignment by examining the utterances produced by speakers: if two agents produced the same utterance \ndg{to refer to a given target?}, we assign a 1, otherwise we assign a 0.
\ndg{need to say we look at alignment at the end of period with given partner (time 4,8,12)? also that it's average across objects?}
...
\ndg{i expect some reviewers will complain about the alignment measure we used here -- eg is non-empty intersection strong enough test? especially given that it's weaker than the exact match we required in the model.... we could leave this as something to deal with in revision, or we could add alternative measures. hausdorf distance between sets of content words, in glove space?}
