...
Throughout the following simulations, we set $w_I = 11,~w_C = 7$. 
A grid search over parameter space revealed other regimes of behavior, but we leave broader exploration of this space to future work.
...
Each network was then randomly assigned one of three distinct "contexts" containing abstract tangram stimuli taken from \cite{clark_referring_1986}.
...
A frequent concern in prior work using repeated reference games is that improvements in communication over time are due to generic effects of task familiarity rather than interactive adaptation to a partner's language use \cite{HupetChantraine92_CollaborationOrRepitition}.
...
The effects we observe at partner boundaries show that something is being learned beyond pure familiarity with the task: if speakers and listeners were just learning to better describe and identify targets regardless of their partner is, we would not expect these reversions.
These partner-specificity effects clearly rule out the complete pooling model with a practice effect confound, but cannot rule out a no-pooling model with a practice effect confound: under this model, partner-specificity effects are real, but the general decrease in utterance length and increase in accuracy with new partners is due to practice rather than inductive generalization.

The best evidence against this hypothesis lies in our network convergence results.
Under the no-pooling model with a practice effect, each speaker and listener would be expected to each individually get better at efficient description and accurate identification across partners, but not to tailor those efficient descriptions to the apparent and emerging conventions of their network.
The fact that the network as a whole converges to similar short descriptions across partners, and that different networks converge to different descriptions, is thus inconsistent with a pure practice effect.
...
Thus, while our behavioral data is inconsistent with complete-pooling and no-pooling accounts, there remain deep theoretical connections between our hierarchical Bayesian formulation and alternative theories of generalization \cite{rogers2004semantic,marcus2018algebraic,doumas2008theory}.
...
Alternatively, if different partners are represented as different "contexts" \cite{brown2015people}, then appropriately hierarchichal reinforcement learning mechanisms may produce similar predictions \cite{gershman2015novelty}.
