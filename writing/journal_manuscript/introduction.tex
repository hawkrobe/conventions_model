...
Computational accounts seeking to explain how conventions emerge do not typically allow for partner-specific meaning at all: agents are assumed to update a monolithic representation of language \cite{Steels}.
...
In this paper, we propose an inferential theory of convention formation that reconciles community-level consensus and partner-specific common ground.
...
We argue that three core cognitive mechanisms are needed for an agent to solve this problem: (1) \emph{uncertainty about what words mean to different partners}, (2) \emph{rapid adaptation across interaction with particular partners}, and (3) \emph{inductive generalization to new partners}.
%\begin{enumerate}
...
%\end{enumerate}
Further, we show that a hierarchical Bayesian model satisfies these desiderata, thereby grounding convention formation in the same generic cognitive processes responsible for success in concept learning, causal learning, and speech recognition\footnote{One way of motivating this model is an argument from the continuity of probabilistic models of language acquisition in development \cite<e.g.>{XuTenenbaum07_WordLearningBayesian,FrankGoodmanTenenbaum09_Wurwur}. Our model can be seen as generalizing Bayesian models of word learning to the interactive referential setting among mature language users. As such, it instantiates a view of convention formation as \emph{meta-learning}, gradually abstracting away the structure that is shared across many extended ddyadic interactions with different partners.}
Finally, we show that this model successfully explains three key phenomena in the empirical literature on repeated reference games that have proved evasive for previous accounts: (1) reduction to more efficient referring expressions in dyadic interaction, (2) contetxt-sensitivity in which conventions form, and (3) partner-specificity effects in the emergence of conventions on small networks.
