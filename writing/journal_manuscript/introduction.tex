...
This theory is motivated by the computational problems facing agents who must communicate with one another in a variable and non-stationary world. 
We argue that three core cognitive capacities are needed for an agent to solve this problem: 
\begin{description}
\item[M1:] the ability to represent \textbf{uncertainty} about what words will mean to different partners
\item[M2:] online \textbf{learning} to coordinate on \emph{ad hoc} conventions with particular partners
\item[M3:] inductive \textbf{generalization} to abstract away stable expectations for new partners
\end{description}
%\begin{enumerate}
...
%\end{enumerate}
Finally, we present a hierarchical Bayesian model that satisfies these desiderata and demonstrate that this account successfully explains three key phenomena in the empirical literature that have proved evasive for previous accounts: 
\begin{description}
\item[P1:] convergence to increasingly \textbf{efficient} referring expressions with a single partner
\item[P2:] \textbf{context-sensitivity} in which labels become \emph{ad hoc} conventions
...
In the next section, we introduce the \emph{repeated reference game} paradigm at the center of this literature, review the empirical evidence for each of these phenomena, and briefly discuss some limitations of previous accounts.

\section{Empirical evidence from repeated reference games}
...
On each trial, the speaker is asked to produce a referring expression $u$ --- typically a noun phrase --- that will allow the listener to select the intended target object $o^*\in\mathcal{C}$ from among the other objects in the context.
...
These observations set up our first puzzle of \emph{ad hoc} convention formation in dyads: how does a word or short phrase that would have been ineffective for communicating under the global conventions of a language take on local meaning over mere minutes of interaction? 

\begin{figure}[t!]
\centering
\includegraphics[scale=1.2]{./figures/clark92}
\vspace{1em}
\caption{\textit{Classic phenomena in repeated reference games.} Across multiple repetitions of the same referent with the same partner, speakers converge to increasingly efficient utterances (reps.~1-6). When the listener is  replaced by a new, naive partner, speakers display a key signature of partner-specificity, reverting to longer utterances before converging again with their new partner (reps.~7-12). Reproduced from Table 3 in \protect\citeA{wilkes-gibbs_coordinating_1992}.}
\label{fig:clark92}
\end{figure}
...
While intruiging, this work was limited by an extremely small sample size ($N = 4$ groups) and technical challenges facing the measurement of conventions in the graphical modality \cite<see>[for a different approach]{hawkins2019disentangling}.
