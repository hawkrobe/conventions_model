...
These meanings are \emph{social conventions} in the sense that they are arbitrary to some degree, but sustained by stable prior expectations each person has about others in their community \cite{lewis_convention:_1969, hawkins2019emergence}.
...
\item (M2) rapid \textbf{adaptation} to particular partners
...
\begin{enumerate}
\item (P1) gradual convergence to more efficient referring expressions in dyadic interaction with a single partner, 
\item (P2) context-sensitivity in which conventions form
\item (P3) the pattern of partner-specificity and generalization in the emergence of conventions in small networks.
\end{enumerate}
In the remainder of this section, we introduce the \emph{repeated reference game} paradigm at the center of this literature, review the empirical evidence for these phenomena, and briefly discuss some limitations of previous accounts.
...
On each trial, the speaker is asked to produce a referring expression --- typically a noun phrase --- that will allow the listener to select the intended target from the distractors in context.
Critically, a \emph{repeated reference game} asks them to refer to these same objects across multiple rounds as they build up a shared history of interaction, or \emph{common ground,} with their partner.
Unlike general studies of referring expression generation, studies of coordination and convention require novel, ambiguous stimuli that participants do not already have strong conventions for.
This design allows observation of how referring expressions to the same unfamiliar object change as a function of interaction with a partner.
