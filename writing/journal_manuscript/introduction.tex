...
\todo[]{Be even clearer about how we're restricting to this specific \emph{goal}, and may be different (I think this is now done (have added a footnote about lengthening for politeness, etc--feel free to delete if you want to) } 
A core function of language is \emph{reference}: using words to convey the identity of an entity in the environment. 
Loosely inspired by \citeA{wittgenstein2009philosophical}, empirical studies of coordination and convention in communication have predominantly focused on the subset of language use captured by simple ``reference games.'' In a reference game task, participants are assigned to speaker and listener roles and shown a context  of possible referential targets (e.g. images).
...
Critically, a \emph{repeated reference game} asks speakers to refer to the same targets multiple times as they build up a shared history of interaction with their partners (see Table \ref{table:parameters} in Appendix for a review of different axes along which the design varies).
...
How does a word or short description that would be largely ineffective in an initial conversation take on local meaning over mere minutes of interaction?\footnote{We recognize that referring expressions can be lengthened to satisfy functions other than reference, such as politeness (\textit{Professor Davidson vs. Don}), affection (\textit{Donny vs Don}), or emphasis (\textit{the one and only}). These functions, however, are not particularly relevant in the repeated reference games that are our current focus.}
...
In our final section, we report new empirical data from a dyadic repeated reference task with the context manipulated, where simulated agents and human participants are shown exactly the same sequence of trials.
