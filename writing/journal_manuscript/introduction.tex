...
Influential computational accounts explaining how social conventions emerge in populations \cite<e.g.>{skyrms2010signals,steels2011modeling,barr_establishing_2004,young_evolution_2015} \ks{There are a bunch of older papers on the emergence of signalling systems going back to at least Hurford 1989, give me a shout if you want some additional references} typically do not allow for partner-specific meaning at all.
...
Critically, a \emph{repeated reference game} asks them to refer to the same targets multiple times as they build up a shared history of interaction, or \emph{common ground,} \ks{is common ground == a shared history of interaction? You will know this stuff much better than me but can't we also presuppose common ground due to e.g. the fact that we are both English-speaking academics?} with their partner (see Table \ref{table:parameters} in Appendix for a review of the different axes along which the design varies).
...
And unlike agent-based simulations of convention formation on large networks \cite<e.g.>{steels2011modeling,barr_establishing_2004,centola_spontaneous_2015}, which typically match agents with a new partner for each trial, repeated reference games ensure that participants maintain the same partner throughout an extended interaction.
...
These final messages are as short or shorter than the messages participants choose to allow \emph{themselves} to choose the target in the future \ks{I know what you mean but this sentence was quite hard to follow. Something like: These final messages are as short or shorter than the messages participants produce when they are instructed to generate a description which they themselves will have to interpret in the future}  \cite{FussellKrauss89_IntendedAudienceCommonGround} and are often incomprehensible to overhearers who were not present for the initial messages \cite{SchoberClark89_Overhearers}.
...
Without partner-specific representations, we would expect speakers to continue using the short labels they had converged on with their first partner; instead, speakers reverted to the longer utterances they had initially used and coordinated on new \ks{just out of interest, do you know if these are actually genuinely new, rather than just recapitulating the description they used with the old partner?} \emph{ad hoc} conventions with their new partner (see Fig. \ref{fig:clark92}).
