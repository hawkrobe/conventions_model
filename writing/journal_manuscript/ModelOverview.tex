...
For $\alpha_S = 1$, this decision rule corresponds to Luce's choice axiom \cite{luce1959individual}, but at larger settings of $\alpha_S$, the speaker is more likely to choose the single utterance that maximizes the utility.
\begin{align}
...
That is, an agent should know that more people share the meaning of a word like \emph{dog} than the meaning of specialized medical terms like \emph{sclerotic aorta} \cite{Clark98_CommunalLexicons}, and conversely, should expect more consensus about which form is appropriate to use for familiar concepts than for new or ambiguous concepts.
...
%\adele{or "without any mechanism to privilege certain utterances over others"}
early errors may 
...
%\adele{if rephrasing above is used, what follows could be included, just beginning with (even in a footnote): At the algorithmic level, decay can be...}
...
First, Gricean agents assume that their partner is using language in a cooperative manner.
...
%\adele{: what's above  is about %To address the existence of AMBIGUITY (which massively exists if you include polysemy), you need to appeal to contextual disambiguation of the terms.
