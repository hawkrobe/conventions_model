...
In the RSA formulation of referential communication, a pragmatic speaker denoted by $S_1$ must choose an utterance $u$ that will allow their partner to choose a particular target object $o$ from the current communicative context $\mathcal{C}$. \ks{You frame it as a reference task, labelling objects - isn't it potentially more general than that, or do you think there's something inherent in the maths that makes it a bad model of any kind of meaning transmission?}
...
\end{align}
\ks{I think people might need a bit more help understanding what these mean! I know a bit about how these models work and I found this quite compressed - I think the trick will be to provide enough verbal explanation so we don't lose the bulk of the readership at this point.}
...
This basic framework has accounted for a vast \ks{is it really vast?} range of empirical phenomena in language use \cite{Scontras_problang,KaoWuBergenGoodman14_NonliteralNumberWords,TesslerGoodman16_Generics,LassiterGoodman15_AdjectivalVagueness}.
...
We must have some ability to extend our systems of meaning \ks{you quite often talk about `meaning' when I think you might mean something more general, i.e. meaning and the way it is encoded in linguistic expressions, which I would call `language' rather than `meaning'} on the fly as needed.
...
When all agents are continually learning in this way, we will show that they are not only able to locally coordinate with specific partners but also able to abstract away structure representing shared linguistic conventions \ks{"abstract away structure representing shared linguistic conventions" is super hard to parse!}.
