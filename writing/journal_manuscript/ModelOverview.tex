...
More specifically, we draw upon the recent Rational Speech Act (RSA) framework, which formalizes the Gricean assumption of cooperativity as recursive social inference \cite{frank_predicting_2012,GoodmanFrank16_RSATiCS,FrankeJager16_ProbabilisticPragmatics}.
...
Many minor variations on the basic RSA model have been explored in previous work.
We highlight two design choices in the specific formulation in Eq.~\ref{eq:RSA}. 
First, both the pragmatic speaker and listener are ``action-oriented'' in the sense that they both use a soft-max function to select an action in the reference game, whereas in many instances of prior work, the listener is ``belief-oriented'' and assumed to simply update their beliefs about the world without taking action \cite{qing2015variations}.
Second, we have parameterized the pragmatic speaker and listener functions with a particular lexicon $\phi_k$.

\todo[inline]{TODO: Describe more specific details that are shared across the simulations.
(3) how we ensure normalization is well-defined w/ null utterance (i.e. handling case where a word doesn't apply to any of the objects), (4) , (5) agent's decision rules, i.e. MAP vs. softmax sampling}
