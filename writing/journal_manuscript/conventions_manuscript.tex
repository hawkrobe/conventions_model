...
To explore the robustness of our results in \textbf{P3} to alternative choices, we also considered a weaker prior where the space of possible lexicons allows any denotation to be assigned to any word, including highly redundant and overlapping lexicons (where every object is in the extension of every word), and highly degenerate lexicons (where every word has a completely empty extension).
Given this more unconstrained space of lexicons, the simplest way to penalize complexity is to penalize the \emph{total extension size}, i.e. the summed extensions of all terms.
