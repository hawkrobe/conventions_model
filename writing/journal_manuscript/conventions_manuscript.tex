\documentclass[10pt, man, floatsintext]{apa7}
\usepackage{amssymb}
\usepackage{graphicx}
...
In this paper, we introduce a hierarchical Bayesian theory of coordination and convention formation that aims to reconcile the long-standing tension between these two basic observations.
Under the CHAI (Continual Hierarchical Adaptation through Inference) model, we argue that the central computational problem of communication is not simply transmission, as in classical formulations, but continual \emph{learning} and \emph{adaptation} over multiple timescales.
...
\section*{Appendix C: Example of simplicity prior}

Some models \cite<e.g.>{carr2020simplicity} force the lexicon to strictly partition the set of referents, ensuring that every object must be in the extension of exactly one word.
In this case, a simplicity prior based on extension sizes sounds like it would conflict with a preference for smaller vocabulary sizes: the fewer words you have, the bigger each word's denotation must be, and the simplest lexicon is one word that covers everything. 
Our model, however, does not hard-code such strong constraints and allows, in principle, all possible assignments of denotations to all words (including, e.g. highly redundant and overlapping lexicons where every object is in the extension of every word, as well as highly degenerate lexicons where every word has a completely empty extension). 
In this case, a simplicity prior favoring simpler word meanings necessarily favors smaller lexicons. 
