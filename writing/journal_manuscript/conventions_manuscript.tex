...
\newcommand{\adele}[1]{\textcolor{Red}{[adele: #1]}}  
...
Our approach has been to err on the side of weaker assumptions and allow any stronger preferences to emerge naturally through pragmatic reasoning during learning and usage. 
The space of possible lexicons we consider allows any denotation to be assigned to any word, including highly redundant and overlapping lexicons (where every object is in the extension of every word), and highly degenerate lexicons (where every word has a completely empty extension).
Given this unconstrained space of lexicons, the simplest way to penalize complexity is to penalize the total extension size.
In this case, favoring simpler word meanings necessarily favors smaller lexicons. 
...
To explore the robustness of our results in \textbf{P3}, where the simplicity prior played the largest role, we also ran our simulations using a more common ``partition-based'' prior that imposes stronger assumptions \cite<e.g.>{carr2020simplicity}. 
This prior only considers lexicons that strictly partition the set of referents, ensuring that every object must be in the extension of exactly one word.
In this case, the fewer words you have, the bigger each word's denotation must be, and the simplest lexicon is one word that covers everything.
Simulation results using this prior are shown in \ref{fig:partitionPrior}. 
Note that other choices are possible, such as those only enforcing mutual exclusivity, or enforcing the principle of contrast \cite{clark1987principle}.
...
\centering
  ...
  \caption{Similar simulation results for \textbf{P3} are found when using a stronger partition-based lexical prior, where the space of lexicons is constrained to strictly partition the space of meanings (such that the `smallest' lexicon is one word referring to all 4 objects and the `largest' lexicon is 4 words, each referring to unique objects). Columns and rows of facets represent different values of $\alpha$ and $\beta$, respectively. }
