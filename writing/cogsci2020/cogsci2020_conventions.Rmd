...
Complete-pooling is appropriate for some everyday social interactions, such as coordinating with anonymous drivers on the highway. 
In everyday communicative settings, however, it is less tenable. 
...
We propose a *partial-pooling* account that satisfies these requirements. 
This account assumes that each agent has uncertainty over the system of meaning their current partner is using and updates partner-specific beliefs given observations of their partner’s language use and understanding. 
Critically, unlike complete-pooling or no-pooling models, we also assume agents' uncertainty has hierarchical structure. 
The meanings used by different partners are expected to be drawn from a shared community-wide prior distribution but are also allowed to differ from one another in systematic, partner-specific ways. 
This structure also provides an inductive pathway for abstract population-level expectations to be distilled from partner-specific experience [see also @KleinschmidtJaeger15_RobustSpeechPerception; @tenenbaum_how_2011 for hierarchical accounts of abstract knowledge in other domains]. 
In other words, we suggest that shared meanings result from agents solving a meta-learning problem.

We begin by formalizing this account in a Bayesian model of communication. 
...
```{r model_results, fig.env = "figure", fig.pos = "t", fig.width=4, fig.height=4, fig.align = "center", set.cap.width=T, num.cols.cap=1, fig.cap = "Simulation results."}
read_csv('../../simulations/hierarchical/hierarchicalSimulationOutput.csv') %>%
...
We recruited  participants from Amazon Mechanical Turk to play an interactive, natural-language reference game using the Dallinger platform\footnote{http://docs.dallinger.io/}.
