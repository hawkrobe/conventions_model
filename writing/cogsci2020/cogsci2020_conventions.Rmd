...
We evaluate participants' generalization behavior on the same three metrics we used in our simulations: accuracy, utterance length, and network convergence.
...
In this section, we examine the actual *content* of pacts and measure changes in alignment across network. 
...
Because the latter group is not aware of one another, if they nonetheless happen to tacitly be aligned, this can be taken as evidence of network-wide coordination.
...
We found a significant interaction ($b =$ `r round(convergence.lmer.interaction$estimate,2)`, $t =$ `r round(convergence.lmer.interaction$statistic,2)`, $p<0.001$; see Fig. \ref{fig:results}C).
