...
It has much stronger initial expectations about meaning by its fourth partner ($\sim$ 75\% accuracy vs. 50\% with the first partner.)
...
To evaluate the qualitative predictions observed in our simulations, we designed a group communication experiment.
Instead of anonymizing partners, as in many previous empirical studies of convention formation, we divided the experiment into blocks of extended dyadic interactions with stable, identifiable partners [see @fay_interactive_2010; @garrod_conversation_1994 for earlier examples of this network design].
...
As we showed in our simulations above, our hierarchical Bayesian model predicts that these pacts will revert at partner boundaries, but that agents should be increasingly willing to transfer expectations from one partner to another in their community. 
...
First, we examined changes in listener accuracy over the sequence of partners, where accuracy is defined as the proportion of trials where the target was correctly selected (see Fig. \ref{fig:results}A).
...
To test changes at partner boundaries, we constructed a different regression model.
We coded the blocks before and after each partner swap with categorical variables and compared the final repetition block with one partner against the first repetition block with the next.
