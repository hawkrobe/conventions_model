...
keywords: >
   communication; convention; generalization
...
Given observations $D_k$ from repeated communicative interactions with partner $k$, the agent updates their beliefs about $k$'s latent system of meaning using Bayes rule:
\begin{equation}
\begin{array}{rcl}
...
In the absence of strong information about partner-specific language use departing from this common structure, the agent ought to regularize toward background knowledge of the population's conventions [@davidson_nice_1986].
...
This transfer is sometimes referred to as a "sharing of strength" or "partial pooling" [@gelman2006data] because pooled data is smoothly integrated with domain-specific idiosyncracies depending on the data available.
...
The probability assigned to the target on each trial is shown in Fig. \ref{fig:simulations}A.
...
To allow for reduction, we allowed a set of four primitive utterances, $\{u_1, u_2, u_3, u_4\}$, to be combined into longer conjunctions, e.g. $\{u_1+u_2, u_3+u_4\}$, which are assumed to have a higher utterance cost.
The meanings of these two-word utterances were determined compositionally from the values of the primitive utterances^[We used the standard product T-norm semantics for conjunction in fuzzy logic, where values lie in the $[0,1]$ interval. Because values are drawn from a Gaussian prior with support over the real numbers, we first used a logistic function to map primitive values to the unit interval, and a logit function to map the product back to the original domain.].
Additionally, because speakers are not initially at chance over their *entire* vocabulary, we used a weakly biased prior for $\Theta$: two of the primitive utterances initially applied slightly more strongly to $o_1$ and the other two more strongly to $o_2$.
This weak bias leads to a preference for conjunctions at the outset and thus allowed us to examine the speaker's shifting preference for conjunctions as they strengthen their beliefs.

To focus on the speaker model's behavior, we paired it with a fixed listener who always correctly selected the target, and ran 48 independent simulations. 
First, replicating findings from @hawkins_convention-formation_2017, we found that descriptions become more efficient over interaction with a single partner: the model becomes more confident that the shorter utterances will be meaningful, so the additional informativity provided by the conjunction begins to be not worth the additional cost.
Critically, we find that speakers revert back to a longer description at the first partner swap, just as our listener model did: evidence from a single partner is relatively uninformative about the community-level distribution.
