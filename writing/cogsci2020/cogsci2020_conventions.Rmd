...
```{r model_results, fig.env = "figure*", fig.pos = "t!", fig.width=7, fig.height=3, fig.align = "center", set.cap.width=T, num.cols.cap=1, fig.cap = "\\label{fig:simulations} Listener model expectations across a series of different partners."}
library(grid)
library(gridExtra)
...
In particular, variational inference allows us to amortize inference as additional data is observed. 
We run 50,000 steps of gradient descent on the first observation to obtain a posterior, compute the agent's marginal prediction for the next observation, then continue running gradient descent on the same stored parameters after including the new observation in the data.
...
In our first simulation, we consider a listener agent's shifting expectations about which target is being referred to as they adapt their semantic representations.  used the same simple scenario with two objects and two words as we used to test adaptation to a single partner.
As our measure of interest, we focus on 
