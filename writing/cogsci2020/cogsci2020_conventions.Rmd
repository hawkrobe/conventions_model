...
```{r model_results, cache = T, fig.env = "figure*", fig.pos = "t!", fig.width=6, fig.height=3, fig.align = "center", set.cap.width=T, num.cols.cap=1, fig.cap = "\\label{fig:simulations} Listener model expectations across a series of different partners."}
library(grid)
library(gridExtra)
...
model.plt1 <- read_csv('../../simulations/hierarchical/networkOutput1.csv') %>%
  ggplot(aes(x = time, y = prediction, color = factor(partnerID), group = factor(partnerID))) +
    geom_point() +
    geom_line() +
    geom_hline(yintercept = .5) +
...
Now that all agents are updating contingent on the other's behavior, however, additional dependencies are introduced.
