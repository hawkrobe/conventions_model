...
```{r}
trajectories <- dir(path = "../simulations/cogsci2018/output/", pattern = "game.*_.*csv", full.names = T) %>% # 
...
trajectories %>%
  ...
  ggplot(aes(x = time, y = empirical_stat, color = condition)) +
    ...
    geom_smooth(se=F) +
    facet_grid(paste0('discount:', discount) ~ .) +
...
```{r}
trajectories %>% filter(time < 8) %>% group_by(condition, time) %>% tidyboot::tidyboot_mean(correct, nboot=100)
```
