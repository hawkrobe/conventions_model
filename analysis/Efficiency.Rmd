...
d.conjunction.all %>%
  group_by(speakerAlpha, listenerAlpha, discountFactor, guessingEpsilon, costWeight, repNum) %>%
  summarize(empirical_stat = mean(utt_length)) %>%
  ungroup() %>%
  filter(discountFactor == 0.8, guessingEpsilon == 0.01) %>%
  #filter(speakerAlpha > 10) %>%
  #filter(costWeight < 0.6) %>%
  #filter(discountFactor == 1) %>%
  # mutate(guessingEpsilon = log(guessingEpsilon),
  #        alpha = speakerAlpha) %>%
  mutate(repNum = repNum + 1) %>%
  ggplot(aes(x = repNum, y = empirical_stat, 
             color = guessingEpsilon,
             group = guessingEpsilon)) +
    geom_line() +
    #geom_hline(yintercept = c(2,1), linetype = 'dotted') +
    facet_grid(speakerAlpha ~ costWeight) +
    theme_few() +
    theme(aspect.ratio = 1) +
    cowplot::panel_border(color = "grey95") +
    scale_x_continuous(breaks=c(1,3,5))+
    scale_y_continuous(breaks=c(1,0.5,2), limits = c(0.75, 2.25))+
    labs(y = '# words', x ='repetition #')
