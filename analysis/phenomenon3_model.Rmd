...
```{r}
trajectories <- read_csv(here('simulations/cogsci2018/output/simplified_trajectories_noPrior.csv')) %>%
  # read_csv(here('simulations/cogsci2018/output/trajectory_subOnly_sub_production.csv')) %>%
  # bind_rows(read_csv(here('simulations/cogsci2018/output/trajectory_basicOnly_basic_production.csv'))) %>%
  # bind_rows(read_csv(here('simulations/cogsci2018/output/trajectory_mixed_mixed_production.csv'))) %>%
  # bind_rows(read_csv(here('simulations/cogsci2018/output/trajectory_subOnly_sub_production2.csv'))) %>%
  # bind_rows(read_csv(here('simulations/cogsci2018/output/trajectory_basicOnly_basic_production2.csv'))) %>%
  # bind_rows(read_csv(here('simulations/cogsci2018/output/trajectory_mixed_mixed_production2.csv'))) %>%
  filter(time < 24) %>%
...
```
```{r}
read_csv(here('simulations/cogsci2018/output/simplified_trajectories_revision.csv')) %>%
  group_by(condition, chainNum, iterationNum) %>%
  mutate(firstTrial = first(correct)) %>%
  group_by(condition, firstTrial, time) %>%
  summarize(m = mean(correct)) %>%
  ggplot(aes(x = time, y = m, color = condition)) +
    geom_point() +
    geom_smooth() +
