...
d %>%
  ...
  tidyboot_mean(acc) %>%
  ggplot(aes(x = trialNum, y = 100 * empirical_stat, color = condition)) +
...
```{r}
d %>%
  filter(!(gameid %in% non_converged)) %>%
...
postTest %>%
  ...
  group_by(gameid, condition, finalRole, extensionSize) %>%
  tally() %>%
  mutate(extensionSize = factor(extensionSize)) %>%
  complete(extensionSize, nesting(gameid, condition, finalRole), fill = list(n = 0)) %>%
  filter(extensionSize == 2) %>%
  lmer(n ~ condition + (1 | gameid), data = .) %>%
  summary()
```
